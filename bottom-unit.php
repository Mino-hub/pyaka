<div class="gs">
<style>
@media(max-width: 319px) { .ad_bottom { width: 300px; height: 250px; } }
@media(max-width: 320px)and(max-width: 355px){ .ad_bottom { width: 300px; height: 250px; } }
@media(min-width: 356px)and(max-width: 425px) { .ad_bottom { width: 336px; height: 280px; } }
@media(min-width: 426px)and(max-width: 769px) { .ad_bottom { width: 336px; height: 280px; } }
@media(min-width: 769px)and(max-width: 1024px) { .ad_bottom { width: 728px; height: 90px; } } 
@media(min-width: 1025px)and(max-width: 1366px){ .ad_bottom { width: 728px; height: 90px; } } 
@media(min-width: 1367px){ .ad_bottom { width: 728px; height: 90px; } }
</style>
<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
<!-- unskilled.site記事shitaレスポンシブ -->
<ins class="adsbygoogle ad_bottom"
     style="display:block"
     data-ad-client="ca-pub-2508992942318870"
     data-ad-slot="2340007201"
     data-ad-format="auto"></ins>
<script>
(adsbygoogle = window.adsbygoogle || []).push({});
</script>
</div>
